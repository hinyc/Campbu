"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
var posts_1 = require("../../entity/posts");
var users_1 = __importDefault(require("../../entity/users"));
var users_reviews_1 = require("../../entity/users_reviews");
var reservation_1 = require("../../entity/reservation");
var typeorm_1 = require("typeorm");
var AuthorizeToken_1 = require("../jwt/AuthorizeToken");
exports.default = {
    get: function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
        var id, postRepository, userRepository, reviewRepository, reservationRepository, post, userId, reviews, reservations, user;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    id = Number(req.params.postId);
                    postRepository = (0, typeorm_1.getRepository)(posts_1.posts);
                    userRepository = (0, typeorm_1.getRepository)(users_1.default);
                    reviewRepository = (0, typeorm_1.getRepository)(users_reviews_1.users_reviews);
                    reservationRepository = (0, typeorm_1.getRepository)(reservation_1.reservation);
                    return [4 /*yield*/, postRepository
                            .createQueryBuilder('post')
                            .loadRelationCountAndMap('post.likes_count', 'post.likes')
                            .where('post.id = :id', { id: id })
                            .getOne()];
                case 1:
                    post = _a.sent();
                    return [4 /*yield*/, postRepository
                            .createQueryBuilder()
                            .select('users_id')
                            .where('posts.id = :id', { id: id })
                            .getRawOne()
                            .then(function (res) { return __awaiter(void 0, void 0, void 0, function () {
                            return __generator(this, function (_a) {
                                return [2 /*return*/, Object.values(res)[0]];
                            });
                        }); })];
                case 2:
                    userId = _a.sent();
                    return [4 /*yield*/, reviewRepository
                            .createQueryBuilder()
                            .select(['count', 'reviews_id'])
                            .where('users_reviews.users_id = :users_id', { users_id: userId })
                            .getRawMany()];
                case 3:
                    reviews = _a.sent();
                    return [4 /*yield*/, reservationRepository
                            .createQueryBuilder()
                            .where('reservation.posts_id = :posts_id', { posts_id: post === null || post === void 0 ? void 0 : post.id })
                            .getMany()];
                case 4:
                    reservations = _a.sent();
                    return [4 /*yield*/, userRepository.findOne({ id: userId })];
                case 5:
                    user = _a.sent();
                    if (reviews === undefined || reviews === null) {
                        res.status(200).json({ posts: post, reviews: [], user: user, reservations: reservations });
                    }
                    else {
                        res.status(200).json({ posts: post, reviews: reviews, user: user, reservations: reservations });
                    }
                    return [2 /*return*/];
            }
        });
    }); },
    delete: function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
        var decoded, userRepository, user, id, postRepository;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4 /*yield*/, (0, AuthorizeToken_1.authorizeToken)(req, res)];
                case 1:
                    decoded = _a.sent();
                    return [4 /*yield*/, (0, typeorm_1.getRepository)(users_1.default)];
                case 2:
                    userRepository = _a.sent();
                    return [4 /*yield*/, userRepository.findOne({ email: decoded.email })];
                case 3:
                    user = _a.sent();
                    if (!!user) return [3 /*break*/, 4];
                    res.status(401).json({ message: 'Unauthorized User' });
                    return [3 /*break*/, 6];
                case 4:
                    id = req.params.postId;
                    postRepository = (0, typeorm_1.getRepository)(posts_1.posts);
                    return [4 /*yield*/, postRepository.delete({ id: Number(id) })];
                case 5:
                    _a.sent();
                    res.status(200).json({ message: 'Product Post Successfully Deleted' });
                    _a.label = 6;
                case 6: return [2 /*return*/];
            }
        });
    }); },
};
